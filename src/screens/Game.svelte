<script>
  ...
  const pickMessage = (p) => {
    if (p < 0.5)
      return pickRandom(["Ouch", "Must try harder", "That wasn't very good"]);
...
<header>
  <p>
    Tap on the more monetisable celebrity face, or tap 'same price' if society
    values them equally.
  </p>
</header>
...
<div class="game-container">
  ...
  {:else if ready}
    ...
    {:catch}
      <p class="error">Ooops! Failed to load the data</p>
    {/await}
  {/if}
</div>
